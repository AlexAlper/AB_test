...
def get_top_query(date_from, date_to, number_len, delta_plus=False):
    ...
    query = "SELECT trim(lowerUTF8(search_bar)) as search_bar, COUNT(DISTINCT number) as number_all_ FROM logs.tovs_search WHERE date_add between '{date_1}' and '{date_2}' GROUP BY search_bar"

    df_number_all = get_general_top(query, date_from, date_to)

    query = "SELECT trim(lowerUTF8(search_bar)) as search_bar, COUNT(id_search) as search_all_ FROM logs.tovs_search WHERE date_add between '{date_1}' and '{date_2}' GROUP BY search_bar"

    df_search_all = get_general_top(query, date_from, date_to)

    query = "SELECT trim(lowerUTF8(search_bar)) as search_bar, COUNT(id_search) as add_all FROM logs.tovs_search WHERE date_add between '{date_1}' and '{date_2}' AND id_element in ('add', 'button') GROUP BY search_bar"
    ...
    for num_1, num_2 in number_len:
        ...
        query = "SELECT trim(lowerUTF8(search_bar)) as search_bar, COUNT(1) as search_bar_add_all_ FROM logs.tovs_search WHERE date_add between '{date_1}' and '{date_2}' AND id_element in ('add', 'button') AND number in number_list GROUP BY search_bar"
